...
sys_exofork(void)
{
	...
	#ifdef LT_SCHDLG
		// El proceso hijo tendrá el 90% de los tickets que tiene el padre
		int curenv_priority = sys_get_tkts(curenv->env_id);
		int newenv_priority = curenv_priority * 0.9;
		env_modify_priority(newenv, newenv_priority);
	#endif
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_yield:
		sys_yield(); // No return
	case SYS_set_tkts:
		return sys_set_tkts(a1, (int) a2);
	case SYS_get_tkts:
		return sys_get_tkts(a1);
	default:
		return -E_INVAL;
	}
}
