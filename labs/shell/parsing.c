...
expand_environ_var(char *arg)
{
	...
	char *environ_value = getenv(arg + 1);

	if (environ_value == NULL) {
		strcpy(arg, "");
	} else {
		if (strlen(environ_value) > strlen(arg)) {
			arg = realloc(arg, strlen(environ_value) + 1);
			if (arg == NULL) {
				printf_debug("Error en realloc para Token");
				return NULL;
			}
		}
		strcpy(arg, environ_value);
	}
	return arg;
}
